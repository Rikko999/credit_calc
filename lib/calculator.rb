...
class Calculator
  ...
  def run
    ...
    @collection = (1..credit_period).to_a.map do |month_index|
      case @credit_method
      when 'standart'
        calculate_standart(credit_period, month_index, percent_rate, credit_sum)
      when 'annuity'
        info = calculate_annuity(credit_period, percent_rate, credit_sum, prev_balance)
  ...
  def calculate_standart(credit_period, month, percent_rate, credit_sum)
    ...
    CalculatorData.new(payed_credit_part, payed_percent, payed_full, balance)
  end

  def calculate_annuity(credit_period, percent_rate, credit_sum, prev_balance)
