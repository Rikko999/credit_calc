RSpec.describe Calculator do
  ...
  let(:random_credit_sum) { '%.2f' % ((rand(1_000_000_000) + 1) / 100) }
  let(:random_credit_period) { (rand(360) + 1).to_s }
  let(:random_credit_method) { rand(100) / 2 > 50 ? 'standart' : 'annuity' }

  subject do
    Calculator.new('percent_rate'   => random_percent_rate,
  ...
  describe '#run' do
    ...
    it 'assigns collection' do
      expect(subject.instance_variable_get('@collection')).to match_array([instance_of(CalculatorData)] * subject.credit_period)
    end
  end

  describe '#calculate_standart' do
    ...
    let(:calculate_usual_result) do
      subject.send(:calculate_standart, subject.credit_period, month, subject.percent_rate, subject.credit_sum)
    end

    it 'returns CalculatorData instance' do
      expect(calculate_usual_result).to be_an CalculatorData
    end

    it 'sets valid payed_credit_part for CalculatorData instance' do
      expect(calculate_usual_result.payed_credit_part).to eq(payed_credit_part_result)
    end

    it 'sets valid payed_percent for CalculatorData instance' do
      expect(calculate_usual_result.payed_percent).to eq(payed_percent_result)
    end

    it 'sets valid payed_full for CalculatorData instance' do
      expect(calculate_usual_result.payed_full).to eq(payed_full_result)
    end

    it 'sets valid balance for CalculatotData instance' do
      expect(calculate_usual_result.balance).to eq(balance_result)
    end
  end

  describe '#calculate_annuity' do
    ...
    it 'returns CalculatorData instance' do
      ...
      (1..subject.credit_period).to_a.each do
        calculate_equal_result = subject.send(:calculate_annuity, subject.credit_period, subject.percent_rate, subject.credit_sum, prev_balance)
        expect(calculate_equal_result).to be_an CalculatorData
    ...
    it 'sets valid balance for CalculatorData instance' do
      prev_balance = nil
      (1..subject.credit_period).to_a.each do
        calculate_equal_result = subject.send(:calculate_annuity, subject.credit_period, subject.percent_rate, subject.credit_sum, prev_balance)
